                ...
                <tr class="border-t">
                    ...
                    <td class="px-6 py-4">
                        <div class="flex gap-2">
                            <a href="{{ route('admin_unit.circulations.show', $circulation) }}" class="text-blue-600 hover:text-blue-800">
                                <i class="fas fa-eye"></i>
                            </a>
                            
                            @if($circulation->status == 'pending')
                                <form action="{{ route('admin_unit.circulations.approve', $circulation) }}" method="POST" class="inline">
                                    @csrf
                                    <button type="submit" class="text-green-600 hover:text-green-800">
                                        <i class="fas fa-check"></i>
                                    </button>
                                </form>
                                <form action="{{ route('admin_unit.circulations.reject', $circulation) }}" method="POST" class="inline">
                                    @csrf
                            ...
                            @if($circulation->status == 'approved')
                                <form action="{{ route('admin_unit.circulations.return', $circulation) }}" method="POST" class="inline">
                                    @csrf
